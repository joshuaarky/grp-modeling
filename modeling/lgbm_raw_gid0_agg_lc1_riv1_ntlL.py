...
# === Logging Setup ===
model_id = "lgbm_raw_gid0_agg_lc1_riv1_ntlL"
...
for key, predictors in predictor_sets.items():
    ...
    n_trials = 1 # set low for testing; otherwise, set to 200
    optuna_storage = f"sqlite:////p/projects/impactee/Josh/thesis_analysis/optuna/optuna_{model_id}/optuna_lgbm_{key}.db"
    ...
    n_repeats = 20  # default is 20; set to 1 for testing
    ...
    for rep in range(n_repeats):
        ...
        for fold_idx, (_, test_group_idx) in enumerate(gkf.split(unique_groups, groups=unique_groups)):
            ...
            def coeff_variation(arr, eps=1e-6):
                return np.std(arr) / (np.mean(arr) + eps)

            def quartile_coeff_dispersion(arr, eps=1e-6):
                q1, q3 = np.quantile(arr, [0.25, 0.75])
                return (q3 - q1) / (q3 + q1 + eps)

            for region, group_df in test_df.groupby("GID_1"):
                y_true_region = group_df[target].values
                y_pred_region = group_df["y_pred"].values
                ...
                # Dispersion metrics (based on predictions)
                qcd = quartile_coeff_dispersion(y_pred_region)
                cv = coeff_variation(y_pred_region)
                ...
                region_metrics.append({
                    ...
                    # Store region-level raw values for later full analyses
    ...
    # Combine all per-region metrics
    ...
    X_all = final_preprocessor.fit_transform(df_model[predictors + ['GID_1']])
    y_all = df_model[target].values

    # --- Train final model ---
    final_model = lgb.train(
        {**best_params, "objective": "regression", "metric": "rmse", "verbosity": -1},
        lgb.Dataset(X_all, y_all),
        num_boost_round=1000
    )

    # --- Prepare for SHAP ---
    X_all_dense = X_all.toarray() if hasattr(X_all, "toarray") else X_all
    X_num_only = X_all_dense[:, :len(predictors)]  # numeric-only for summary plots
    X_num_df = pd.DataFrame(X_num_only, columns=predictors)

    # --- Run SHAP ---
    explainer = shap.TreeExplainer(final_model)
    shap_values = explainer.shap_values(X_all_dense)

    # interaction_values = explainer.shap_interaction_values(X_all_dense)
