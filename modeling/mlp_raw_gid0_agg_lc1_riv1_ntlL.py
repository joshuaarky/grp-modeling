...
# === Logging Setup ===
# model_id = "mlp_raw_gid0_agg_lc1_riv1_ntlL"
model_id = "mlp_raw2_gid0_agg_lc1_riv1_ntlL"
# model_id = "ann_adamw_raw2_gid0_agg_lc1_riv1_ntlL"
...
lc_features = [
    "cropland_share", "forest_share", "log_urban_share"
]
...
for key, predictors in predictor_sets.items():
    ...
    class MLPWrapper:
        ...
        def fit(self, X, y=None):
            # No-op fit method so sklearn.partial_dependence accepts this object
            return self
    ...
    # # Paths
    # pdp_path = f"/p/projects/impactee/Josh/thesis_analysis/pdp_plots/{model_id}/"
    # os.makedirs(pdp_path, exist_ok=True)

    # # 1) Fit final preprocessor on full df_model (must match what you used during CV)
    # final_preprocessor = ColumnTransformer([
    #     ('num', StandardScaler(), predictors),
    #     ('cat', OneHotEncoder(drop='first', handle_unknown='ignore'), ['GID_1'])
    # ])
    # # Fit on the full dataset (df_model)
    # X_all_proc = final_preprocessor.fit_transform(df_model[predictors + ['GID_1']]).toarray()

    # # 2) Train a final model on the whole dataset using best_params
    ...
    # optimizer = torch.optim.AdamW(final_model.parameters(), lr=best_params['lr'], weight_decay=best_params.get('weight_decay', 0.0))
    # criterion = nn.MSELoss()

    # # Prepare tensors and dataloader
    # y_all = df_model[target].values.reshape(-1, 1)
    # tensor_X = torch.tensor(X_all_proc, dtype=torch.float32, device=device)
    # tensor_y = torch.tensor(y_all, dtype=torch.float32, device=device)
    # dataset = TensorDataset(tensor_X, tensor_y)
    # loader = DataLoader(dataset, batch_size=best_params['batch_size'], shuffle=True)

    # # Train for a fixed number of epochs (simple final fit)
    # n_final_epochs = 150
    # final_model.train()
    ...
    #         loss = criterion(preds, yb) + final_model.l2_penalty() * best_params.get('l2_lambda', 0.0)
    #         loss.backward()
    #         optimizer.step()
    #         epoch_losses.append(loss.item())
    #     if (epoch + 1) % 50 == 0:
    #         logger.info(f"Final model training epoch {epoch+1}/{n_final_epochs} mean loss: {np.mean(epoch_losses):.6f}")

    # # 3) Prepare wrapper and feature names (from final_preprocessor)
    # wrapped = MLPWrapper(final_model, device=device)
    # # wrapped.fit(None, None)  # satisfies sklearn, does nothing

    # # Use get_feature_names_out for full processed names
    # feature_names_proc = final_preprocessor.get_feature_names_out()
    # feature_names_proc = list(feature_names_proc)  # convert to list for .index()

    # logger.info(f"PDP will use {len(feature_names_proc)} processed features (example: {feature_names_proc[:10]})")

    # # 4) Create directory for this key
    # plot_path = os.path.join(pdp_path, f"pdp_plots_{key}")
    # os.makedirs(plot_path, exist_ok=True)

    # # 5) Loop over original predictors and compute PDP on their processed 'num__' column
    # for pred in predictors:
    #     proc_name = f"num__{pred}"
    #     if proc_name not in feature_names_proc:
    #         logger.warning(f"{proc_name} not found in processed feature names; skipping PDP for {pred}")
    #         continue

    #     idx = feature_names_proc.index(proc_name)

    #     fig, ax = plt.subplots(figsize=(6,4))
    #     PartialDependenceDisplay.from_predictions(
    #         wrapped.predict,  # callable that accepts X and returns predictions
    #         X_all_proc,       # must be dense np.array
    #         features=[idx],
    ...
    #     ax.set_title(f"PDP – {pred}")
    #     plt.tight_layout()
    #     fig_filepath = os.path.join(plot_path, f"pdp_{pred}.png")
    #     fig.savefig(fig_filepath, dpi=150)
    #     plt.close(fig)
    #     logger.info(f"Saved PDP for {pred} to {fig_filepath}")

    # logger.info(f"All PDP plots saved to {plot_path}")
